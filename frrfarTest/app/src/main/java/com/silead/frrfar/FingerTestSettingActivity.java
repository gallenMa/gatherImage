...
public class FingerTestSettingActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener, View.OnClickListener {
    ...
    private CheckBoxPreference imageSizeIconSetting;
    ...
    private static final int MSG_UPDATE_ICON_IMAGE_SIZE = 14;
    @Override
    public void onCreate(Bundle icicle) {
        ...
        imageSizeIconSetting = (CheckBoxPreference) findPreference("finger_settings_size");
        ...
        mStatusUpdateRunnable = new Runnable() {
            public void run() {
                ...
                if(imageSizeIconSetting != null){
                    updatePreferenceStatus(imageSizeIconSetting,MSG_UPDATE_ICON_IMAGE_SIZE);
                }
            }
        };
        ...
        if(imageSizeIconSetting != null){
            imageSizeIconSetting.setOnPreferenceChangeListener(this);
        }
        if (mBackBtn != null) {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_UPDATE_ICON_IMAGE_SIZE:{
                    if(imageSizeIconSetting != null) {
                        Log.e(FingerSettingsConst.LOG_TAG, "fingerImage icon silee"+((Boolean)msg.obj).booleanValue());
                        imageSizeIconSetting.setChecked(((Boolean)msg.obj).booleanValue());
                    }
                }
            }
        }
    ...
    private void updatePreferenceStatus(Preference preference, int msg) {
        ...
        if (preference instanceof EditTextPreference) {
            ...
            }else if(msg == MSG_UPDATE_ICON_IMAGE_SIZE){
                name = FingerSettingsConst.FP_SETTINGS_ICON_IMAGE_SIZE;
                value = FingerSettingsConst.FP_SETTINGS_ICON_IMAGE_SIZE_DEFAULT;
                Log.e(FingerSettingsConst.LOG_TAG, "fingersize test vallue"+value+name);
            }
            if (name != null && !name.isEmpty()) {
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mEnrollSampleCountEdtPref) {
            ...
            updatePreferenceStatus(mFactoryTestClearPwdAutoCBoxPref, MSG_UPDATE_FACTORY_CLEAR_PWD_AUTO_VALUE);
        }else if(preference == imageSizeIconSetting){
            boolean value = ((Boolean)newValue).booleanValue();
            Log.e(FingerSettingsConst.LOG_TAG, "fingerImage icon zie" + value);
            SharedPreferencesData.saveDataBoolean(this,FingerSettingsConst.FP_SETTINGS_CONFIG,
                    FingerSettingsConst.FP_SETTINGS_ICON_IMAGE_SIZE,value);
            updatePreferenceStatus(imageSizeIconSetting,MSG_UPDATE_ICON_IMAGE_SIZE);
        }
        return true;
    }
